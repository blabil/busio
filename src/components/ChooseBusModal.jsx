...
const ChooseBusModal = ({ isOpen, onClose, onSubmit, userID }) => {
    ...
    const fetchBus = async () => {
        const response = await axios.get(`http://localhost:5000/busdriver/getbus/${userID}`, {
            withCredentials: true,
        });
        setBusList(response.data);
        setBus(response.data[0].id);
    };
