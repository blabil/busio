...
const BusRouteEdit = () => {
  ...
  const handleAssignDriverRoute = async (driverID) => {
    let response = null;
    try {
      const temp = await BusRouteService.handleAssingDriverToRoute(
        id,
        driverID
      );
      setRouteInfo({
        ...
        <TwoPartPanel>
          <BasicPanel>
            ...
            {routeInfo.driver ? (
              <div className="flex flex-col justify-content items-center">
                <NavigateHref
                  label={`Kierowca: ${routeInfo.driver.profile.fullName}`}
                  path={`/user/type/${routeInfo.driver.id}`}
                ></NavigateHref>
                <BasicHeader
                  label={"Telefon:"}
          ...
          <BasicPanel>
            ...
            {routeInfo.bus ? (
              <div className="flex flex-col justify-content items-center">
                <NavigateHref
                  label={`Rejestracja: ${routeInfo.bus.registration}`}
                  path={`/bus/edit/${routeInfo.bus.id}`}
                ></NavigateHref>
                <BasicHeader
                  label={"Pojazd:"}
                  value={
