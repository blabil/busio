...
const StopList = () => {
    ...
    const handleDelete = async (stopID) =>{
        let response = null;
        try{
            response = await BusRouteService.handleDeleteStop(stopID);
            fetchStops();
        } catch(error) {
          ...
          <div className="w-auto h-full">
            <h2 className="mb-4 text-3xl font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl dark:text-white">
              ...
              <div className="flex-1 h-full">
                <div onClick={handleOpenModal} className="flex items-center justify-center flex-1 h-full w-full p-2 border border-sky-400 rounded-full ho hover:bg-teal-500 hover:text-white cursor-pointer">
          ...
          <tbody className="bg-white">
            {stops.map((stop) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-center">{stop.address}</td>
